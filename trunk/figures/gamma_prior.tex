% Generated using matlabfrag
% Version: v0.6.16
% Version Date: 04-Apr-2010
% Author: Zebb Prime
%
%% <text>
%
\providecommand\matlabtextA{\color[rgb]{0.000,0.000,0.000}\fontsize{10}{10}\selectfont\strut}%
\psfrag{007}[cl][cl]{\matlabtextA $\Lambda = 10$}%
\psfrag{008}[cl][cl]{\matlabtextA $\Lambda = 0.56$}%
\psfrag{009}[cl][cl]{\matlabtextA $\Lambda = 0.1$}%
\psfrag{010}[bc][bc]{\matlabtextA $p(L)$}%
\psfrag{011}[tc][tc]{\matlabtextA $L$}%
%
%% </text>
%
%% <xtick>
%
\def\matlabfragNegXTick{\mathord{\makebox[0pt][r]{$-$}}}
%
\psfrag{000}[ct][ct]{\matlabtextA $0$}%
\psfrag{001}[ct][ct]{\matlabtextA $1$}%
\psfrag{002}[ct][ct]{\matlabtextA $2$}%
\psfrag{003}[ct][ct]{\matlabtextA $3$}%
%
%% </xtick>
%
%% <ytick>
%
\psfrag{004}[rc][rc]{\matlabtextA $0$}%
\psfrag{005}[rc][rc]{\matlabtextA $0.5$}%
\psfrag{006}[rc][rc]{\matlabtextA $1$}%
%
%% </ytick>